...
+ (void)requestForBulletinContext:(NSPBulletinContext*)context
  ...
  // Multi-target: push to every non-empty endpoint (url/url2/url3). Empty
  // endpoints are treated as disabled and skipped.
  NSArray<NSString*>* urls = @[
    XStrDefault(config.rawPrefs[@"url"], @""),
    XStrDefault(config.rawPrefs[@"url2"], @""),
    XStrDefault(config.rawPrefs[@"url3"], @"")
  ];
  ...
  for (NSString* urlString in urls) {
    ...
    NSPushRequest* request =
        [self requestForBulletinContext:context config:config urlString:urlString];
    if (request) {
...
+ (NSPushRequest*)requestForBulletinContext:(NSPBulletinContext*)context
  ...
  return [self requestForBulletinContext:context
                                  config:config
                               urlString:XStrDefault(config.rawPrefs[@"url"], @"")];
}
