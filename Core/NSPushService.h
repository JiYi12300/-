...
// Multi-target variant: returns MULTIPLE requests (one per configured target)
// so a single channel can push to several endpoints simultaneously (e.g. two
// Bark devices / two HTTP webhooks). The base default wraps the synchronous
// single-request builder into a one-element array; services that support
// several targets (Bark, HTTP, ServerChan) override this to return one request
// per non-empty configured target.
+ (void)requestForBulletinContext:(NSPBulletinContext*)context
                           config:(NSPushServiceConfig*)config
...
// Multi-target variant: substitutes REPLACE_KEY using a SPECIFIC key (one of
// several per channel). Used by Bark/ServerChan to build a distinct URL per
// configured target (key / key2 / key3 share the same URL template).
+ (NSString*)replacedKeyURLStringForConfig:(NSPushServiceConfig*)config
                                       key:(NSString*)key;
