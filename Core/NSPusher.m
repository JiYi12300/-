      ...
      dispatch_get_global_queue(QOS_CLASS_USER_INITIATED, 0), ^{
        ...
        [serviceClass
            requestForBulletinContext:context
                             ...
                             requests:^(NSArray<NSPushRequest*>* requests) {
                               if (requests.count == 0) {
                                 // Service failed to build any request (e.g.
                                 // WeChat couldn't fetch an access token, or
                                 // every configured target was empty); log the
                               ...
                               // A channel may target several endpoints at
                               // once (e.g. two Bark devices / two HTTP
                               // webhooks); send each assembled request. The
                               // service is responsible for fully assembling
                               // each request (URL, headers, body, method,
                               // body encoding).
                               for (NSPushRequest* request in requests) {
