...
+ (void)requestForBulletinContext:(NSPBulletinContext*)context
  ...
  // Default: single-request services keep their existing behavior, wrapped
  // into a one-element array. Services with multiple targets override this.
  [self requestForBulletinContext:context
                           config:config
                       completion:^(NSPushRequest* request) {
                         requests(request ? @[ request ] : nil);
                       }];
}
